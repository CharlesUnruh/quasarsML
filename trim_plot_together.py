...
def cleanLocMin(filename):
    ...
    for i in range(3):
        #print(len(samples[:,i]))
        keep_indecies = doubleMADsfromMedian(samples[:,i],thresh=4.0)
...
for file in filenames:
    ...
    include = True
    for vals in samples_IR:
        for val in vals:
            if False:
                include = False
    if include:
        all_samples.extend(samples_IR)
    else:
        print("Parameters exceed 10, not including")
    i += 1
fig = corner.corner(all_samples, labels=[r"$\alpha_1$", r"$\alpha_2$", r"$\beta_0$",r"$\beta_1$"],
                    truths=[0.75, 0.01,7.0e-9,1.2e-9],quantiles=[0.16, 0.5, 0.84],
                    show_titles=True, title_kwargs={"fontsize": 15})
fig.savefig("plotted_together.png")
plt.close(fig)
